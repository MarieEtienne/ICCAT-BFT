...
caasurvey <- strtoi(caasurvey[seq(1,length(caasurvey), 1+nage-sage+2)])+1
caaData <- c(rep(1, nyr-syr), caasurvey)

## Bspline if caa data available, logistic selectivity if not
outf[count<- count+1] <- paste(paste(5*(1:ngear %in% caaData)+1-(1:ngear %in% caaData), collapse='\t'), '\t# 1  -selectivity type ivector(isel_type) for gear')
...
outf[count<- count+1] <- paste(paste( round( nOccurrences(caaData, vect2=1:ngear) * 1 + 1 ), collapse='\t'), '\t# 6  -Phase of estimation (-1 for fixed)')
