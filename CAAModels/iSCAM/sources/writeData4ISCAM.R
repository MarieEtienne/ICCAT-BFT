...
outf[count <- count+1] <- paste('\t', linf, '\t\t# -asymptotic length (linf)', sep='')
outf[count <- count+1] <- paste('\t', k,'\t\t# -brody growth coefficient (k)', sep='')
...
for( ind in (ns+1):(ns+(nyr-syr+1)) ){
  ...
  compositionCatch[ind-ns, 3:(nage-sage+3)] = substring[2:length(substring)] / sum( substring[2:length(substring)])
}
...
relIndices <- as.numeric(unlist(strsplit(vpaData[(ns+1):nf],'\t')))
relIndices <- relIndices[!is.na(relIndices)]
