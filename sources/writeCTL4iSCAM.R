...
#outf[count<- count+1] <- paste(paste(1*(1:ngear %in% caaData)+6*( ! (1:ngear %in% caaData)), collapse='\t'), '\t# 1  -selectivity type ivector(isel_type) for gear')
#logistic for JP_LL and Bspline for commercial
outf[count<- count+1] <- paste(paste(c(3,1), collapse="\t"), '\t# 1  -selectivity type ivector(isel_type) for gear')
...
#                               '\t# 2  -Age/length at 50% selectivity (logistic)')
outr[countr <- countr+1] <- paste(unlist(strsplit(outf[count],"\t")), collapse=" ")
outf[count<- count+1] <- paste(paste(1*( (1:ngear) != 4)+0.11*(  (1:ngear)==4 ), collapse='\t'),
